...
import { Menu, Volume2, VolumeX, Clock, MapPin, ChevronDown, LogOut } from "lucide-react";
...
export function Topbar({ pageTitle, onMenuToggle }) {
  ...
  const { city, cities, canChangeCity, setCity } = useCityScope();
  ...
  const [soundEnabled, setSoundEnabled] = useState(() => {
    return localStorage.getItem("nimbus_audio_alert") === "true";
  });
  ...
  const toggleSound = () => {
    ...
    localStorage.setItem("nimbus_audio_alert", next ? "true" : "false");
  };
  ...
  return (
    <header className="topbar">
      <div className="topbar-left">
        ...
        {isDashboard ? (
          <div className="topbar-dashboard-title">
            <span className="topbar-dashboard-name">NIMbus</span>
      ...
      <div className="topbar-right">
        {role === "admin" && (
          <div className="topbar-city-selector">
            <MapPin size={13} />
            <span className="topbar-city-label">CITY</span>
            <select
              aria-label="Select city scope"
              value={city}
        ...
        <div className="topbar-user-section">
          {user?.username && (
            <span className="topbar-username" title={user.email || user.username}>
              {user.username}
            </span>
          )}
          <button
