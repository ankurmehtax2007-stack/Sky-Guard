...
export function StationMonitorGrid({ scopedCity = null, scopedStationId = null, readOnly = false }) {
  ...
  const filteredReadings = useMemo(() => {
    return readings.filter((r) => {
      if (scopedStationId && r.stationId !== scopedStationId) return false;
      if (scopedCity && getStationCity(r).toLowerCase() !== scopedCity.toLowerCase()) return false;
      if (search) {
        const q = search.toLowerCase();
        const matchId = (r.stationId || "").toLowerCase().includes(q);
        const city = getStationCity(r).toLowerCase();
        if (!matchId && !city.includes(q)) return false;
      }
  ...
  }, [readings, filter, search, scopedCity, scopedStationId]);
  ...
  return (
    <div>
      <div className="section-header" style={{ flexWrap: "wrap", gap: "10px" }}>
        <div className="section-header-left">
          <span className="section-title-lg">{scopedCity ? `${scopedCity} AWS Network` : "Automatic Weather Station Network"}</span>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "12px", flexWrap: "wrap" }}>
          ...
          {!readOnly && <Link to="/stations" className="card-link" style={{ marginLeft: "4px" }}>
            All Stations →
          </Link>}
        </div>
      </div>
