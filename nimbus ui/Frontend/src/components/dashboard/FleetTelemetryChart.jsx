...
const STORAGE_KEY = "nimbus_fleet_telemetry_history";
...
function TelemetryChart({ config, data, stations }) {
...
export function FleetTelemetryChart() {
  ...
  useEffect(() => {
    ...
    const fetchFleetData = async () => {
      try {
        ...
        const incoming = {};
        const addIfInScope = (stationId, result) => {
          if (city !== "All Cities" && getStationCity(stationId).toLowerCase() !== city.toLowerCase()) return;
          if (result.status === "fulfilled" && result.value?.data?.readings?.length) {
            incoming[stationId] = result.value.data.readings;
          }
        };
        addIfInScope("AWS_01", r1);
        addIfInScope("AWS_02", r2);
        addIfInScope("AWS_03", r3);
  ...
  const stations = useMemo(
    () => Object.keys(readingsData).filter((stationId) =>
      city === "All Cities" || getStationCity(stationId).toLowerCase() === city.toLowerCase()
    ),
    [readingsData, city]
  );
  const chartData = useMemo(() => {
    return Object.fromEntries(
      SENSOR_CONFIG.map((sensor) => [
        sensor.key,
        buildChartData(
          Object.fromEntries(stations.map((stationId) => [stationId, readingsData[stationId]])),
          sensor.key
        ),
      ])
    );
